...
/*
you need to color, other doesnt --> color those first
if you can color yours + shared before they color theirs, win; else tie
*/
void run() {
    int n; cin >> n; VLL v(n); INP(v,n);
    safe_set<int> shared,p1,p2;
    rep(i,0,n) {
        if (v[i] != i+1) p1.insert(i);
        if (v[i] != n-i) p2.insert(i);
    ...
    dbg(sz(p2));
    dbg(sz(shared));
    int a = sz(p1),b=sz(p2),c=sz(shared);
    if (a+c<=b) {
        print("First");
    } else if (b+c<a) {
        print("Second");
