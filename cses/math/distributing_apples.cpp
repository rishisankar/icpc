...
/*
ct(n+1,m) 
= ct(n,m)  +ct(n,m-1)   +ct(n,m-2) + ...     k=0
= ct(n-1,m)+2ct(n-1,m-1)+3ct(n-1,m-2)+...    k=1
= ct(n-2,m)+3ct(n-2,m-1)+6ct(n-2,m-2)+...    k=2
...
= T_{1, n-1} ct(1,m) + T_{2,n-1} ct(1,m-1) + ...
= T_{1,n-1} + ... + T_{m+1,n-1}


ct(n,m) = T_{1,n-2} + ... + T_{m+1,n-2}
T_{a,b} = (a+b-1) choose b
ex: n=3, m=2
1 choose 1 + 2 choose 1 + 3 choose 1

ct(3,2)
= ct(2,2) + ct(2,1) + ct(2,0)
= ct(1,2)+2ct(1,1)+3ct(1,0)


ct(n,m) = ct(n-1,m) + ct(n-1,m-1) + ... + ct(n-1,0)
ct(n,m-1) = ct(n-1,m-1) + ... + ct(n-1,0)
...
ct(n,0) = ct(n-1,0)
...
void run() {
    ...
    if (n == 2) {print(m+1); return;}
    mod_int ans = 0;
    rep(i,0,m+1) {
        ans += choose(i+n-2, n-2);
    }
    print(ans);
}
