...
void run() {
    ...
    int ct = 0;
    if (cp(s1,s2)) swap(s1,s2);
    if (cp(s2,s3)) swap(s2,s3);
    if (cp(s1,s2)) swap(s1,s2);
    dbg(s1,s2,s3);
    while (!(sz(s1) == 1 && sz(s2) <= 2 && sz(s3) <= 2 && s2.back() != s3.back())) {
        if (cp(s1,s2)) swap(s1,s2);
        if (cp(s2,s3)) swap(s2,s3);
        if (cp(s1,s2)) swap(s1,s2);
        if (s1.back() == s2.back()) {
    ...
    if (s1.back() != -3 && s2.back() != -3 && s3.back() != -3) ++ct;
    print(ct);
}
