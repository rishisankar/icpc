...
void run() {
    ...
    mt19937 g(rd());
    shuffle(all(v), g);
    // dp [i] [j] = at current prefix, max # ppl u can use to have sumA - sumB = i, pA - pB = j
    constexpr int BD = 150;
    vector<unordered_map<int, int>> dp(BD+1); 
    dp[0][0] = 0;
    rep(X,0,n) {
        vector<unordered_map<int, int>> dpnew = dp;
        rep(i, 0, BD+1) {
            for (auto p : dp[i]) {
                int j = p.first;
                int mx = p.second;
                ...
                // case 1: v[X] joins team A
                if (i < BD) updmax(dpnew[i+1][j + v[X]], mx + 1);
                // case 3: v[X] joins team B
                if (i > 0) updmax(dpnew[i-1][j - v[X]], mx + 1);
                else updmax(dpnew[1][v[X] - j], mx + 1);
            }
        }
        swap(dp, dpnew);
        // dbg(dp[0][0], X);
