...
void run() {
    ...
    INP(v,n);
    int mxd = 5000;
    vector<double> dist(mxd);
    dist[0] = 1;
    rep(i,0,n) {
        mxd+=1010;
        vector<double> ndist(mxd);
        rep(J,0,sz(dist)-1005) {
            ndist[J + 1002] += dist[J] * v[i];
            if (J >= 1001) {
                ndist[J-1000] += dist[J] * (1-v[i]);
            } else {
                ndist[J] += dist[J] * (1-v[i]);
            }
        }
        swap(dist,ndist);
    }
