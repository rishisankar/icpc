...
void run() {
    ...
    rep(i,0,n) {
        string s; cin >> s;
        if (s[0] == 'e') v[i].F = -1; else v[i].F = 1;
        ll z = stoll(s.substr(1,sz(s)-1));
        if (nm.count(z)) z = nm[z];
        else z = nm[z] = sz(nm);
        v[i].S = z;
    }
    int m = sz(nm);
    ...
    rep(i,0,n) {
        csum[v[i].S] += v[i].F;
        msum[v[i].S] = min(msum[v[i].S], csum[v[i].S]);
    }
    int tot = 0;
    rep(i,0,m) if (csum[i] == 0 && msum[i] == 0) ++tot;
    // dbg(1,tot);
    pii best = {1,tot};
    rep(i,0,n-1) {
        int j = v[i].S;
        if (csum[j] == 0 && msum[j] == 0) --tot;
        msum[v[i].S] -= v[i].F;
        if (csum[j] == 0 && msum[j] == 0) ++tot;
        // dbg(i+2,tot);
        if (tot > best.S) best = {i+2,tot};
    }
    print(best.F,best.S);
}
