...
// return best increase in MST by removing edge from st at t to par (or internal)
ll solve(int t) {
    ll bst = 0;
    for (int c : chd[t]) {
        bst = max(bst, solve(c));
    }
    // build set of edges coming out of t
    for (auto ed : aed[t]) {
        sets[t]->insert(ed);
    }
    for (int c : chd[t]) {
        sets[t] = merge(sets[t], sets[c]);
    }

    if (t == 1) dbg(sets[t]->size());
    if (sets[t]->size() >= 2) {
        auto it = sets[t]->begin();
        ll w1 = it->F;
        advance(it,1);
        ll w2 = it->F;
        bst = max(bst, w2-w1);
