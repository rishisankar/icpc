        ...
        rep(j,0,2*m) {
            ...
            if (zz.F.F > 0 && zz.F.S > 0 && zz.S.F > 0 && zz.S.S > 0) {
                ...
                rep(i,0,sz(pts)) {
                    stack<pii> s2;
                    VVI vis2(n, VI(2*m,-1));
                    s2.push(pts[(i+1)%sz(pts)]);
                    vis2[s2.top().F][s2.top().S] = cc;
                    vis2[pts[i].F][pts[i].S] = cc;
                    pair<pii,pii> zz2 = {{0,0},{0,0}};
                    while (!s2.empty()) {
                        pii t = s2.top(); s2.pop();
                        pair<pii,pii> et = edge_touch(t.F,t.S);
                        zz2.F.F += et.F.F;
                    ...
                    if (zz2.F.F > 0 && zz2.F.S > 0 && zz2.S.F > 0 && zz2.S.S > 0) {
                        fail(); return;
                    }
                }
                print(cct);
