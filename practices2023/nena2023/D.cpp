...
void run() {
    ...
    rep(i,0,n) {
        ...
        if (l1.F == -1 || l2.F == -1) {
            ll scale = 10000;
            P z1(s1.x + scale * (s1.x-f1.x), s1.y + scale * (s1.y-f1.y));
            P z2(s2.x + scale * (s2.x-f2.x), s2.y + scale * (s2.y-f2.y));
            vector<P> rect{z1,z2,s2,s1};
            ll area2 = llabs(polygonArea2(rect));
            ll bpts = acps(s1,s2) + acps(s1,z1) + acps(s2,z2) + acps(z1,z2) - 4;
            ll ipts = (area2 - bpts + 2) / 2;
            if (ipts != 0) return inf();
            else continue;
        }
        ...
        if (i1 == i2) {
            ...
            int SO1 = sideOf(rem[0],f1,s2);
            int SO2 = sideOf(rem[1],f2,s1);
            int SO3 = sideOf(rem[0],rem[1],f2) * (-1);
            assert(SO1 != 0);
            assert(SO2 != 0);
            assert(SO3 != 0);
            ...
            for (ll xp = xmin; xp <= xmax; ++xp) {
                for (ll yp = ymin; yp <= ymax; ++yp) {
                    P cpt(xp,yp);
                    int SOP1 = sideOf(rem[0],f1,cpt);
                    if (SOP1 != SO1) continue;
                    int SOP2 = sideOf(rem[1],f2,cpt);
                    if (SOP2 != SO2) continue;
                    int SOP3 = sideOf(rem[0],rem[1],cpt);
