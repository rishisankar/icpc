...
void run() {
    ...
    rep(i,0,n/2) {
        match = false;
        dbg(cp, i+1, n-i-1, n-1-i + (i+1-cp));
        dbg(hi.hashInterval(cp, i+1) == hi.hashInterval(n-1-i, n-1-i + (i+1-cp)));
        if (hi.hashInterval(cp, i+1) ==  hi.hashInterval(n-1-i, n-1-i + (i+1-cp))) {
            cp = i+1;
            ++ans;++ans;
            match = true;
            dbg(i, "match");
